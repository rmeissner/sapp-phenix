...
const Rebirth: React.FC<Props> = ({ manager, module }) => {
  ...
  const loadHetchlings = useCallback(async () => {
    try {
      ...
      for (const e of events) {
        ...
        if (!announcementInfo || (announcementInfo.execTime <= (new Date().getTime() / 1000) && !announcementInfo.executed)) {
      ...
      setHetchlings(Object.values(results).filter(announcement => !announcement.executed && announcement.execTime > 0))
    } catch (e) {
      console.error(e)
      setHetchlings([])
    }
  ...
  const breed = useCallback(async (executor) => {
    setBreeding(true)
    try {
      ...
      const announcementDetails: AnnouncementDetails = {
        executor, to: executor, value: BigNumber.from(0), data, operation: 0, nonce: BigNumber.from(new Date().getTime())
      }
      const hashImage = await module.generateTransactionHashData(
        announcementDetails.executor, announcementDetails.to, announcementDetails.value, announcementDetails.data, announcementDetails.operation, announcementDetails.nonce
      )
  ...
  const hetch = useCallback(async (txHash) => {
    try {
      console.warn({txHash})
  ...
  return (<>
    ...
    {hetchlings?.map(hetchling => <div>
      {hetchling.executor}
      {hetchling.execTime < now && <Button size="md" color="primary" onClick={() => hetch(hetchling.txHash)}>Hetch</Button>}
    ...
    {phenixes?.map(phenixes => <div>
      {phenixes.executor}
      {!breeding && <Button size="md" color="primary" onClick={() => breed(phenixes.executor)}>Breed</Button>}
