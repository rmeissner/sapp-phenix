...
const Rescuers: React.FC<Props> = ({ module, enabled }) => {
  ...
  const addRescuer = useCallback(async () => {
    try {
      await module.updateConfig(newRescuer.trim(), 60, true);
      setNewRescuer('');
    } catch (e) {
      console.error(e);
    }
  }, [newRescuer, setNewRescuer, module]);

  const removeRescuer = useCallback(
    async (announcer) => {
      try {
        await module.updateConfig(announcer, 0, false);
      } catch (e) {
        console.error(e);
      }
    },
  ...
  return (
    <>
      ...
      {enabled && (
        <div>
          <TextField value={newRescuer} label="New Rescuer" onChange={(e) => setNewRescuer(e.target.value)}></TextField>
          <Button size="md" color="primary" onClick={addRescuer}>
            Add
          </Button>
        </div>
      )}
      ...
      {rescuers?.map((rescuer) => (
        <div key={rescuer.announcer}>
          {rescuer.announcer} ({numberZoHHMMSS(rescuer.delay.toNumber())} delay)
          <Button size="md" color="primary" onClick={() => removeRescuer(rescuer.announcer)}>
            Remove
          </Button>
        </div>
      ))}
